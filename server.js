...
app.get('/api/me', auth, (req, res) => {
  const u = db.prepare('SELECT id, username, created_at FROM users WHERE id = ?').get(req.user.id);
  res.json({ user: u });
});
...
app.post('/api/playlists', auth, (req, res) => {
  ...
  const info = db.prepare('INSERT INTO playlists (user_id, nombre) VALUES (?, ?)').run(req.user.id, nombre);
  const pl = db.prepare('SELECT id, nombre, created_at FROM playlists WHERE id = ?').get(info.lastInsertRowid);
...
app.put('/api/playlists/:id', auth, (req, res) => {
  ...
  const pl = db.prepare('SELECT id, nombre, created_at FROM playlists WHERE id = ?').get(id);
  res.json({ playlist: pl });
});
...
app.post('/api/playlists/:id/songs', auth, (req, res) => {
  ...
  const exists = db.prepare('SELECT 1 FROM playlists WHERE id = ? AND user_id = ?').get(id, req.user.id);
...
app.delete('/api/playlists/:id/songs/:songId', auth, (req, res) => {
  ...
  db.prepare('DELETE FROM playlist_songs WHERE id = ? AND playlist_id = ?').run(Number(songId), Number(id));
  res.json({ ok: true });
});
...
console.log('Tiene index.html?', fs.existsSync(path.join(publicDir, 'index.html')));

app.use('/', express.static(publicDir));

// Fallback: si no hay index.html, intentá con biblioteca.html / inicio.html / home.html
app.get('/', (req, res) => {
  const candidates = ['index.html', 'biblioteca.html', 'inicio.html', 'home.html'];
  const found = candidates.find(f => fs.existsSync(path.join(publicDir, f)));
  if (!found) {
    return res
      .status(404)
      .send('No se encontró página de inicio. Busqué: ' + candidates.join(', ') + ' en ' + publicDir);
  }
